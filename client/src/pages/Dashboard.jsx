...
export default function Dashboard() {
  ...
  const [morosas, setMorosas] = useState([]);
  ...
  const fetchReport = async () => {
    try {
      const res = await axios.get(
        "http://localhost:4000/api/reports/houses-status"
      );
      setAlDia(res.data.alDia);
      setMorosas(res.data.morosas);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const fetchLowTanks = async () => {
    try {
      const res = await axios.get(
        "http://localhost:4000/api/inventory/tanks/summary?days=7"
      );
  ...
  const fetchData = async (m = 6) => {
    try {
      const res = await axios.get(
        `http://localhost:4000/api/reports/summary?months=${m}`
      );
      setData(res.data);
    } catch (err) {
      console.error(err);
  ...
  const inventoryCategories = (data?.inventoryByCategory ?? []).map(
    (i) => i._id || "Sin categoría"
  );
  const inventoryValues = (data?.inventoryByCategory ?? []).map(
    (i) => i.totalQty
  );
  ...
  const inventoryOptions = {
    ...
    series: [{ name: "Cantidad", data: inventoryValues }],
  };
        ...
        <div className="col-md-4 mb-3">
          ...
          >
            <div className="card-body">
              ...
              <h2>{totalLowTanks}</h2>
              {totalLowTanks > 0 && (
                <p>{lowTanks.map((t) => t.name).join(", ")}</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="row mt-4">
        <div className="col-md-6 mb-3">
          <div
            ...
            style={{ background: "#28a74515", height: 200 }}
          >
            <div
              className="card-body d-flex flex-column p-3"
              style={{ minHeight: 0 }} // 🔑 CLAVE
            >
              <h6 className="card-title text-success d-flex align-items-center gap-2 mb-1">
                <FaCheckCircle />
                Casas al día
              </h6>
              ...
              >
                <ul className="list-group list-group-flush">
                  {alDia.map((h) => (
                    <li
                      key={h._id}
                      className="list-group-item bg-transparent d-flex justify-content-between align-items-center py-1"
                    >
        ...
        <div className="col-md-6 mb-3">
          <div
            className="card shadow-sm border-0"
            style={{ background: "#dc354515", height: 200 }}
          >
            <div
              className="card-body d-flex flex-column p-3"
              style={{ minHeight: 0 }} // 🔑 CLAVE
            >
              <h6 className="card-title text-danger d-flex align-items-center gap-2 mb-1">
                <FaExclamationTriangle />
                Casas con deuda
              </h6>
                  ...
                  {morosas.map((h) => (
                    ...
                    >
                      ...
                      <span className="badge bg-danger d-flex align-items-center gap-1 small">
                        <FaMoneyBillWave />${h.pendingAmount.toFixed(2)}
                      </span>
                    </li>
                  ))}
                </ul>
