...
router.get("/house/:houseId", auth, async (req, res) => {
  const items = await Payment.find({ house: req.params.houseId }).sort({
    date: -1,
  });
  res.json(items);
});
...
router.post("/", auth, isAdmin, async (req, res) => {
  try {
    ...
    if (idempotencyKey) {
      try {
        p = await Payment.findOneAndUpdate(
          { idempotencyKey, house: req.body.house },
          { $setOnInsert: { ...req.body, idempotencyKey } },
          { new: true, upsert: true }
        );
      } catch (err) {
        if (err.code === 11000)
          p = await Payment.findOne({ idempotencyKey, house: req.body.house });
        else throw err;
      }
    } else {
      // normalize targets if passed in body (for consistent idempotency across re-orderings)
      const normalizedTargets = Array.isArray(req.body.targets)
        ? [...req.body.targets].sort((a, b) =>
            String((a && a.id) || a) > String((b && b.id) || b) ? 1 : -1
          )
        : [];
      const key = crypto
        .createHash("sha256")
        .update(
          JSON.stringify({
            ...
            prepaidBotellones: req.body.prepaidBotellones,
      ...
      try {
        p = await Payment.findOneAndUpdate(
          { idempotencyKey: key, house: req.body.house },
          { $setOnInsert: { ...req.body, idempotencyKey: key } },
          { new: true, upsert: true }
        );
      } catch (err) {
        if (err.code === 11000)
          p = await Payment.findOne({
            idempotencyKey: key,
            house: req.body.house,
          });
        else throw err;
    ...
    if (
      ...
      const normalizedTargets = [...targets].sort((a, b) =>
        String((a && a.id) || a) > String((b && b.id) || b) ? 1 : -1
      );
      const applyKey = crypto
        .createHash("sha256")
        .update(
          JSON.stringify({ applyToOldest: false, targets: normalizedTargets })
        )
        .digest("hex");
      // Reserve apply operation on the created payment
      const reserved = await Payment.findOneAndUpdate(
        { _id: p._id, appliedKeys: { $ne: applyKey } },
        { $push: { appliedKeys: applyKey } },
        { new: true }
      );
      if (reserved) {
        if (hasTransactions) {
          ...
          try {
            await session.withTransaction(async () => {
              const payment = await Payment.findOne({ _id: p._id }).session(
                session
              );
              let rem = payment.amount || 0;
              for (const t of targets) {
                if (rem <= 0) break;
                const applied = await applyPaymentToTarget(
                  payment,
                  t,
                  rem,
                  session
          ...
          if (targetIds.length)
            await Payment.updateMany(
              { _id: { $in: targetIds }, amount: { $lte: 0 }, settled: false },
              {
                $set: {
                  settled: true,
                  settledBy: p._id,
    ...
    console.error("[ERROR] apply payment", err && err.message);
...
router.post("/:id/apply", auth, isAdmin, async (req, res) => {
  try {
    ...
    if (!p.confirmed) {
      console.log("[DEBUG] Payment not confirmed", p._id);
      return res
        .status(400)
        .json({ error: "Payment must be confirmed to apply" });
    }
    let remaining = p.amount || 0;
    console.log("[DEBUG] Remaining for apply", remaining);
    if (remaining <= 0) {
      console.log("[DEBUG] No remaining for payment", p._id, remaining);
    ...
    if (applyToOldest && remaining > 0) {
      ...
      const applyKeyOldest = crypto
        .createHash("sha256")
        .update(
          JSON.stringify({ applyToOldest: true, paymentId: String(p._id) })
        )
        .digest("hex");
      const reservedOldest = await Payment.findOneAndUpdate(
        { _id: p._id, appliedKeys: { $ne: applyKeyOldest } },
        { $push: { appliedKeys: applyKeyOldest } },
        { new: true }
      );
      if (!reservedOldest) {
        p = await Payment.findById(p._id);
        ...
        console.log(
          "[DEBUG] applyToOldest path executed for payment",
          p._id,
          "remaining",
          remaining
        );
        ...
        for (const pend of pendings) {
          ...
          if (remaining >= pendAmount) {
            const updated = await Payment.findOneAndUpdate(
              { _id: pend._id, confirmed: false, settled: false },
              {
                $set: {
                  settled: true,
                  settledBy: p._id,
    ...
    console.log(
      "[DEBUG] Applied total to track on payment",
      String(p._id),
      "appliedTotal",
      appliedTotal
    );
    if (appliedTotal > 0) {
      ...
      await Payment.findOneAndUpdate(
        { _id: p._id },
        { $inc: { appliedAmount: appliedTotal } },
        { new: true }
      );
      ...
      try {
        p = await Payment.findById(p._id);
        console.log(
          "[DEBUG] Reloaded payment appliedAmount",
          p && p.appliedAmount
        );
      } catch (err) {
        console.error(
          "[ERROR] Failed reloading payment after apply",
          err && err.stack
        );
        throw err;
      }
