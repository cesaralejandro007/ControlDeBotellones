...
router.get('/summary', auth, async (req, res) => {
  try {
    ...
    const deliveriesAgg = await Delivery.aggregate([
      { $match: { date: { $gte: sixMonthsAgo } } },
      { $group: { _id: { year: { $year: '$date' }, month: { $month: '$date' } }, total: { $sum: '$count' } } },
      { $sort: { '_id.year': 1, '_id.month': 1 } }
    ])
    ...
    res.json({ paymentsByMonth: paymentsAgg, deliveriesByMonth: deliveriesAgg, inventoryByCategory: invAgg })
