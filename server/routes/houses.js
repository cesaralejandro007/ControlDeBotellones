...
router.get("/:id/detail", auth, async (req, res) => {
  try {
    ...
    const used = deliveries
      .filter((d) => d.usedPrepaid)
      .reduce((s, d) => s + (d.count || 0), 0);
    res.json({
      house: h,
      payments,
      deliveries,
      balance: { prepaid, used, balance: prepaid - used },
    });
  } catch (err) {
...
router.get("/:id/debt", auth, async (req, res) => {
  try {
    ...
    const botellon = await Product.findOne({
      $or: [{ category: "Botellones" }, { type: "botellon" }],
    });
  ...
  try {
    ...
    if (idempotencyKey) {
      ...
      try {
        p = await Payment.findOneAndUpdate(
          { idempotencyKey, house: h._id },
          {
            $setOnInsert: {
              house: h._id,
              amount: amount || 0,
              description,
              prepaidBotellones: prepaidBotellones || 0,
              confirmed: true,
              idempotencyKey,
              reference,
              bank,
      ...
      try {
        p = await Payment.findOneAndUpdate(
          { idempotencyKey: key, house: h._id },
          {
            $setOnInsert: {
              house: h._id,
              amount: amount || 0,
    ...
    if (Array.isArray(targets) && targets.length > 0) {
      ...
      if (!isAdminRole)
        return res
          .status(403)
          .json({ error: "No autorizado para aplicar pagos" });
...
router.put("/:id", async (req, res) => {
  const h = await House.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
  });
  res.json(h);
});
