...
router.get('/house/:houseId', auth, async (req, res) => {
  const items = await Delivery.find({ house: req.params.houseId }).sort({ date: -1 });
  res.json(items);
});
...
router.post('/', auth, isAdmin, async (req, res) => {
  try {
    ...
    if (usedPrepaid) {
      ...
      if (available < count) return res.status(400).json({ error: 'No hay suficientes botellones prepagados para esta entrega' });
    }
    ...
    if ((tankProduct.quantity || 0) < litersNeeded) return res.status(400).json({ error: 'No hay suficiente agua en el tanque para esta entrega' })
    ...
    if (!usedPrepaid) {
      try{
        ...
        const unitPrice = tankModel?.pricePerFill || (botellonProduct?.price || 0)
        const amount = unitPrice * count
        const crypto = require('crypto')
        const key = crypto.createHash('sha256').update(JSON.stringify({ house: String(house), amount, description: `Deuda por entrega (${count} botellones)`, deliveryId: String(d._id) })).digest('hex')
        try{
          await Payment.findOneAndUpdate({ idempotencyKey: key, house }, { $setOnInsert: { house, amount, description: `Deuda por entrega (${count} botellones)`, prepaidBotellones: 0, confirmed: false, idempotencyKey: key } }, { new: true, upsert: true })
        } catch(err) {
          if (err.code === 11000) {
    ...
    res.json({ delivery: d, product: botellonProduct, litersUsed: litersNeeded, pricePerFillUsed: tankModel?.pricePerFill || botellonProduct?.price, litersPerBottle })
